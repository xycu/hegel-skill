...
promptfoo SLM evals for English, Polish, German, Latin, and the light multilingual
smoke set. The eval stages SHALL run sequentially, one language at a time, never
concurrently against the local model. (The former custom eval-runner unit stage is
...
- **AND** the runner SHALL execute the German promptfoo SLM evals
- **AND** the runner SHALL execute the Latin promptfoo SLM evals
- **AND** the runner SHALL execute the light multilingual promptfoo SLM evals
- **AND** the eval stages SHALL run sequentially, not concurrently.
...
- **AND** the German stage SHALL run the German promptfoo eval test set
- **AND** the Latin stage SHALL run the Latin promptfoo eval test set
- **AND** the multilingual stage SHALL run the light multilingual promptfoo eval test set.
