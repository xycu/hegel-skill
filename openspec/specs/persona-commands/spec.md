# persona-commands Specification

## Purpose
Give the persona a deterministic command surface: a `/brandt` summon command that engages
the persona with the exact semantics of the manual-summon rung in
`soused-hegelian-persona`, and a `/brandt-dismiss` command that ends the sticky session as
a sincere dismissal does — both as thin vehicles over the existing activation ladder, never
duplicating the persona engine.
## Requirements
